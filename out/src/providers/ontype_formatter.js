...
function calculateIndent(lines) {
    ...
    while (true) {
        ...
        if (char === '(') {
            const tokens = lines[y].slice(x + 1).split(' '), first = tokens[0], second = tokens.length > 1 ? tokens[1] : null;
            openBrackets.push(oneIndentForms.includes(first)
                ? (x + 2)
...
class ClojureOnTypeFormattingEditProvider {
    ...
    provideOnTypeFormattingEdits(document, position, ch, options) {
        ...
        let rangeUptoHere = new vscode.Range(new vscode.Position(0, 0), position), lines = document.getText(rangeUptoHere).split('\n'), indent = calculateIndent(lines), startPosition = position.with(position.line, 0), endPosition = position;
...
exports.ClojureOnTypeFormattingEditProvider = ClojureOnTypeFormattingEditProvider;
function toggleAutoAdjustIndentCommand() {
    console.log("toggle toggle");
    // state.cursor.set("autoAdjustIndent", !state.deref().get("autoAdjustIndent"));
    // status.update();
}
exports.toggleAutoAdjustIndentCommand = toggleAutoAdjustIndentCommand;
